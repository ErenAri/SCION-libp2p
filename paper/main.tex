...
\subsection{Three-Way Policy Comparison}

Table~\ref{tab:comparison} presents the core comparison results at $N = 10$ nodes.

\begin{table}[t]
\caption{Three-way policy comparison ($N = 10$ nodes, 20 requests, 1\,MB content). Values are means over 5 runs.}
\label{tab:comparison}
\centering
\small
\begin{tabular}{l@{\hspace{6pt}}r@{\hspace{6pt}}r@{\hspace{6pt}}r@{\hspace{6pt}}r@{\hspace{6pt}}r}
\toprule
Policy & Avg (ms) & P50 (ms) & P95 (ms) & P99 (ms) & MB/s \\
\midrule
$\varepsilon$-greedy & 12.3 & 11.0 & 18.5 & 22.1 & 8.21 \\
Latency & 11.8 & 10.5 & 24.2 & 31.5 & 7.95 \\
Random & 18.7 & 17.2 & 28.9 & 35.3 & 5.43 \\
\bottomrule
\end{tabular}
\end{table}

The key finding is in the tail latency: epsilon-greedy achieves \textbf{23.6\% lower P95 latency} and \textbf{29.8\% lower P99 latency} than greedy min-RTT, while having only 4.2\% higher average latency. This is because greedy selection causes transient congestion on the ``best'' path, producing latency spikes that dominate the tail distribution. Epsilon-greedy's 10\% exploration distributes some traffic to alternative paths, smoothing out these spikes.

Random selection has the highest latency across all percentiles because it wastes capacity on high-latency paths. However, it has lower tail-to-median ratio than greedy, confirming that the herd effect is a greedy-specific problem.

\subsection{Scalability}

Figure~\ref{fig:scalability} shows P95 latency as a function of node count for each policy.
...
\addplot[mark=square*, thick, blue] coordinates {(5, 14.2) (10, 18.5) (25, 24.8)};
\addlegendentry{$\varepsilon$-greedy}

\addplot[mark=triangle*, thick, red, dashed] coordinates {(5, 15.1) (10, 24.2) (25, 42.7)};
\addlegendentry{Latency (greedy)}

\addplot[mark=o, thick, green!60!black, dotted] coordinates {(5, 22.3) (10, 28.9) (25, 38.5)};
\addlegendentry{Random}
\end{axis}
\end{tikzpicture}
\caption{P95 latency vs. node count. Greedy min-RTT degrades rapidly at scale due to herd effects. Epsilon-greedy scales gracefully.}
\label{fig:scalability}
\end{figure}

At $N = 5$, the three policies perform similarly in tail latency (14--22\,ms). As node count increases to 25, greedy min-RTT's P95 latency increases by \textbf{183\%} (from 15.1 to 42.7\,ms), while epsilon-greedy increases by only \textbf{75\%} (from 14.2 to 24.8\,ms). This confirms the herd effect: as more nodes compete for the ``best'' path, congestion grows superlinearly. Epsilon-greedy's exploration distributes load, preventing this collapse.

Random selection scales moderately (72\% increase) because it naturally distributes load, but starts from a higher baseline due to its inability to exploit path quality information.
