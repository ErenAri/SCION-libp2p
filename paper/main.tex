...
Thompson & 25.1 & 22.0 & 48.5 & 60.2 & 38.50 \\
Contextual & 26.9 & 23.5 & 52.8 & 65.1 & 36.21 \\
...
At $N{=}5$ with warm-start probes, all policies perform within the same order of magnitude (15--40\,ms average), with random and greedy min-RTT performing slightly better due to the low contention at this scale. Thompson sampling achieves the best tail latency among bandit policies (P95 = 48.5\,ms) thanks to its natural exploration-exploitation balance via posterior sampling. The contextual bandit (LinUCB) performs comparably to UCB1, with the additional advantage of context-awareness that should benefit heterogeneous network conditions. The key insight is that at small scale, path selection policy matters less because contention is low---the differences emerge at larger scales (Section~\ref{sec:scalability}).

\subsection{Scalability}
\label{sec:scalability}

Table~\ref{tab:scalability} and Figure~\ref{fig:scalability} show how P95 latency scales from $N{=}5$ to $N{=}25$ nodes for all seven policies, with warm-start probes (3\,s interval, 5\,s warm-up).
...
Thompson & 48.5 & 30.8 & 57.4 & $+18\%$ \\
Contextual & 52.8 & 33.2 & 61.5 & $+16\%$ \\
...
\addplot[mark=star, thick, brown] coordinates {(5, 48.5) (10, 30.8) (25, 57.4)};
\addlegendentry{Thompson}

\addplot[mark=pentagon*, thick, cyan] coordinates {(5, 52.8) (10, 33.2) (25, 61.5)};
...
\end{axis}
\end{tikzpicture}
\caption{P95 latency vs.\ node count (warm-start probes, localhost TCP). Random degrades by 109\% from $N{=}5$ to $N{=}25$, while bandit policies remain stable. At $N{=}25$, Thompson sampling achieves the lowest P95 (57.4\,ms).}
\label{fig:scalability}
\end{figure}

The key scalability finding is the \textbf{divergence at $N{=}25$}: random selection, which performed best at small scale ($N{=}5$, P95 = 43.1\,ms), degrades by \textbf{109\%} to 89.9\,ms at $N{=}25$. In contrast, the bandit-based policies remain stable---Thompson sampling P95 changes only 18\% (48.5 $\to$ 57.4\,ms), decaying-epsilon by 10\% (54.2 $\to$ 59.7\,ms), and UCB1 by 21\% (51.2 $\to$ 62.2\,ms). The contextual bandit (LinUCB) degrades by only 16\%, showing that context-awareness provides modest additional stability. Greedy min-RTT also degrades (15\%, to 74.3\,ms). At $N{=}25$, all five bandit policies outperform both greedy and random, with Thompson sampling achieving the lowest P95 latency (57.4\,ms).
...
We presented \textsc{scion-libp2p}, a path-aware P2P content overlay that applies multi-armed bandit path selection, NDN-style relay caching with cooperative Bloom filter exchange, Reed-Solomon erasure coding, multipath bandwidth aggregation, and adaptive chunking to improve content delivery over libp2p. Our evaluation across seven policies---including Thompson sampling with Bayesian Beta priors and contextual bandits (LinUCB)---and up to 25 nodes shows that bandit-based selection scales gracefully: Thompson sampling P95 degrades by only 18\% from $N{=}5$ to $N{=}25$, compared to 109\% for random and 15\% for greedy. Erasure coding achieves equivalent fault tolerance to full replication at 1.5$\times$ storage overhead. Popularity-aware caching adapts to skewed content distributions with a 38\% improvement over pure LRU, and proactive replication ensures 100\% content availability under 50\% node failure. The Docker Compose WAN simulation validates these findings under heterogeneous latency conditions.
